...
@section('content')
    ...
    <div class="relative w-full flex items-center justify-center bg-gray-900" style="height: 550px;">
        ...
        <div class="relative z-10 text-left w-full max-w-6xl px-6">
            <h1 class="text-5xl md:text-6xl font-extrabold uppercase tracking-wide"
                style="color: #ffffff !important; text-shadow: 0 4px 6px rgba(0,0,0,0.6);">CONTACT US</h1>
        </div>
    </div>
        ...
        <div class="flex flex-row gap-4 sm:gap-12 md:gap-16 items-start w-full">
            ...
            <div class="flex-1 w-1/2">
                <div class="mb-4 sm:mb-8 text-left">
                    <h2 class="text-[#0f172a] font-bold mb-1 sm:mb-2 text-[22px] sm:text-3xl md:text-[34px] tracking-tight">
                        Get in touch</h2>
                    <p class="text-gray-500 text-[11px] sm:text-sm md:text-[15px] font-medium leading-tight">We are here for
                        you! How can we help?</p><br>
                </div>

                <form action="{{ route('contact.submit') }}" method="POST">
                    @csrf
                    @if(session('success'))
                        <div class="mb-4 p-3 bg-green-50 border border-green-200 text-green-700 rounded-md text-sm font-medium">
                            {{ session('success') }}
                        </div>
                    @endif
                    @if(session('error'))
                        <div class="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 rounded-md text-sm font-medium">
                            {{ session('error') }}
                        </div>
                    @endif
                    ...
                    <div class="mb-4 sm:mb-6">
                        <textarea name="message" rows="4" required placeholder="Go ahead, we are listening.."
                            ...
                            style="background-color: #f0f4f5; box-shadow: inset 0 0 0 1px transparent;"
                            onfocus="this.style.boxShadow='inset 0 0 0 2px #2b6d64'"
                            onblur="this.style.boxShadow='inset 0 0 0 1px transparent'"></textarea>
                    </div>
                    <div class="text-left">
                        <button type="submit"
                            class="text-white font-semibold text-[12px] sm:text-[15px] py-2 sm:py-3.5 px-6 sm:px-10 rounded-full transition-colors inline-block w-auto"
                            style="background-color: #2b6d64;" onmouseover="this.style.backgroundColor='#1f524a'"
                            onmouseout="this.style.backgroundColor='#2b6d64'">
                            Submit
                        </button>
                    </div>
                </form>
            ...
            <div class="flex-1 w-1/2 flex flex-col items-center">
                ...
                <div class="w-full mb-4 sm:mb-10 flex justify-center">
                    <img src="{{ asset('images/contact_iilustration.png') }}"
                        class="w-full h-auto max-w-[300px] object-contain" style="max-height: 220px;"
                ...
                <div class="space-y-3 sm:space-y-6 w-full max-w-[300px] mx-auto">
                    <div class="flex items-center gap-2 sm:gap-5">
                        <div
                            class="w-6 h-6 sm:w-10 sm:h-10 rounded-full border-[1.5px] border-[#2b6d64] flex items-center justify-center shrink-0">
                            <i data-lucide="map-pin" class="w-3 h-3 sm:w-[18px] sm:h-[18px] text-[#2b6d64]"></i>
                        </div>
                        <span class="text-gray-600 font-semibold text-[10px] sm:text-[15px] break-words">205A Millennium
                            City, Accra</span>
                    </div>
                    <div class="flex items-center gap-2 sm:gap-5">
                        <div
                    ...
                    <div class="flex items-center gap-2 sm:gap-5">
                        ...
                        <span
                            class="text-gray-600 font-semibold text-[10px] sm:text-[15px] break-words">phnes.travels@gmail.com</span>
                    </div>
                </div>
            </div>
    ...
    <div class="max-w-6xl mx-auto px-6 py-12 text-center">
        <h2 class="font-extrabold text-gray-900 mb-2" style="font-size: 38px; letter-spacing:-0.5px;">Why Travel With Us?
        </h2>
        <p class="text-gray-500 font-medium text-base mb-12">The best booking platform you can trust</p>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <!-- Card 1 -->
            <div class="px-6 py-10 flex flex-col items-center text-center space-y-4 rounded-3xl"
                style="background-color: #e6f9f6;">
                <div class="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-2">
                    <div class="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center text-yellow-500"><i
                            data-lucide="shield-check" size="18" class="fill-yellow-500"></i></div>
                </div>
                <h4 class="font-extrabold text-gray-900 text-[17px]">Security Assurance</h4>
                <p class="text-gray-500 leading-relaxed font-medium" style="font-size: 13px;">Demonstrates commitment to
                    user data security through encryption and secure payment practices</p>
                <a href="#"
                    class="font-bold mt-2 flex items-center gap-1 transition-colors text-gray-900 hover:text-primary text-xs">Learn
                    More <i data-lucide="arrow-right" size="12"></i></a>
            </div>
            <!-- Card 2 -->
            <div class="px-6 py-10 flex flex-col items-center text-center space-y-4 rounded-3xl"
                style="background-color: #fdf0f4;">
                <div class="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-2">
                    <div class="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center text-red-500"><i
                            data-lucide="user" size="18" class="fill-red-500"></i></div>
                </div>
                <h4 class="font-extrabold text-gray-900 text-[17px]">Security Assurance</h4>
                <p class="text-gray-500 leading-relaxed font-medium" style="font-size: 13px;">Demonstrates commitment to
                    user data security through encryption and secure payment practices</p>
            ...
            <div class="px-6 py-10 flex flex-col items-center text-center space-y-4 rounded-3xl"
                ...
                <div class="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-2">
                    <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-500"><i
                            data-lucide="package" size="18" class="fill-blue-500"></i></div>
                </div>
                <h4 class="font-extrabold text-gray-900 text-[17px]">Security Assurance</h4>
                <p class="text-gray-500 leading-relaxed font-medium" style="font-size: 13px;">Demonstrates commitment to
                    user data security through encryption and secure payment practices</p>
            ...
            <!-- Card 4 -->
            <div class="px-6 py-10 flex flex-col items-center text-center space-y-4 rounded-3xl"
                style="background-color: #f6f3fc;">
                <div class="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-2">
                    <div class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-500"><i
                            data-lucide="file-text" size="18" class="fill-indigo-500"></i></div>
                </div>
                <h4 class="font-extrabold text-gray-900 text-[17px]">Security Assurance</h4>
                <p class="text-gray-500 leading-relaxed font-medium" style="font-size: 13px;">Demonstrates commitment to
                    user data security through encryption and secure payment practices</p>
    ...
    <section class="py-16 lg:py-24 overflow-hidden border-t border-gray-100 relative" style="background-color: #eef6ff;">
        ...
        <div class="absolute top-10 pointer-events-none hidden md:block"
            style="left: 15%; width: 250px; height: 120px; opacity: 0.8; z-index: 10;">
