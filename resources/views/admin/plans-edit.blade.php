...
<div class="space-y-6 pb-12" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="space-y-1">
            ...
            <h2 class="font-black text-foreground tracking-tight text-3xl">Edit Subscription Plan</h2>
            <p class="text-muted-text font-medium text-sm">Modify parameters, quotas, and subscription details for this plan tier.</p>
        </div>
        <div>
    ...
    <!-- Main Content Grid -->
    <form action="{{ url('/admin/plans/update') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        @csrf
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-100">
                    ...
                    <div class="flex items-center relative">
                        ...
                        <input required type="number" step="1" name="price" x-model="price" value="{{ (int)$plan->price }}" placeholder="149" class="w-48 bg-[#F8F9FA] border border-gray-200 rounded-lg py-2 pl-3 pr-12 outline-none text-xs text-right font-semibold text-foreground focus:border-primary" />
                    </div>
                </div>
                        ...
                        <div class="flex justify-between text-xs font-bold">
                            ...
                            <span class="text-gray-800">
                                @php
                                    ...
                                    echo DB::table('agents')->where('tier', 'like', '%' . $tier . '%')->count();
                                @endphp
                            </span>
                        </div>
                        <div class="flex justify-between text-xs font-bold">
                            <span class="text-muted-text">Revenue (MTD)</span>
                            <span class="text-primary font-black" style="color: #e85d26 !important;">
                                ₹{{ number_format(DB::table('agents')->where('tier', 'like', '%' . $tier . '%')->count() * $plan->price, 2) }}
                            </span>
                        </div>
                        <div class="flex justify-between text-xs font-bold">
                            <span class="text-muted-text">Last Modified</span>
                            <span class="text-gray-800">
                                {{ $plan->updated_at ? \Carbon\Carbon::parse($plan->updated_at)->diffForHumans() : 'Just now' }}
                            </span>
                        </div>
                    </div>
                </div>
