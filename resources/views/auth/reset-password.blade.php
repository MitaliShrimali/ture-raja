    ...
    <div class="flex min-h-screen" x-data="{ showPassword: false, showPasswordConfirm: false }">
        ...
        <div class="w-full lg:w-[45%] flex flex-col justify-center px-8 md:px-16 lg:px-24 py-12">
            <div class="max-w-md w-full mx-auto space-y-10">
                <div class="space-y-4">
                    <h2 class="font-black text-foreground text-3xl tracking-tight">Reset Password</h2>
                    <p class="text-xs font-bold text-primary uppercase tracking-widest mb-1">Create a new password for your account.</p>
                </div>
                <!-- Flash Messages -->
                @if(session('error'))
                    <div
                ...
                @endif
                <!-- Main Form -->
                <form action="{{ url('/reset-password') }}" method="POST" class="space-y-8">
                    @csrf
                    <input type="hidden" name="token" value="{{ $token }}">
                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black text-muted-text uppercase tracking-widest pl-1">New Password<span
                                class="text-primary">*</span></label>
                        <div class="relative">
                            <input required :type="showPassword ? 'text' : 'password'" name="password" minlength="8"
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black text-muted-text uppercase tracking-widest pl-1">Confirm Password<span
                                class="text-primary">*</span></label>
                        <div class="relative">
                            <input required :type="showPasswordConfirm ? 'text' : 'password'" name="password_confirmation" minlength="8"
