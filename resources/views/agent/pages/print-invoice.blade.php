...
<head>
    ...
    <title>Invoice {{ $invoiceData['invoice_no'] }} - Tour Raja</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700;800&display=swap');
        body { font-family: 'Poppins', sans-serif; color: #1e293b; background-color: #f8fafc; }
...
<body class="py-6 px-4">
    ...
    <div class="max-w-4xl mx-auto flex items-center justify-between gap-4 bg-white p-4 mb-5 rounded-2xl border border-gray-200 shadow-sm no-print">
        <div class="flex items-center gap-3">
            <a href="{{ (request()->is('admin/*') || Str::contains(url()->current(), 'admin')) ? url('/admin/payments') : route('agent.invoice') }}" class="p-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-gray-600 transition-all flex items-center text-xs font-bold gap-2 cursor-pointer">
                <i class="fas fa-arrow-left"></i> {{ (request()->is('admin/*') || Str::contains(url()->current(), 'admin')) ? 'Back to Payments' : 'Back to Invoices' }}
            </a>
            <div>
                <h3 class="text-sm font-bold text-gray-900">Invoice {{ $invoiceData['invoice_no'] }}</h3>
                <p class="text-xs text-gray-500 font-medium">Dated: {{ $invoiceData['invoice_date'] }}</p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <button id="agent-pdf-btn" onclick="downloadPDF(event)" style="background-color: #2e7d32 !important;" class="cursor-pointer hover:opacity-90 text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-md flex items-center gap-2">
                <i class="fas fa-download"></i> Download PDF
            </button>
            <button onclick="window.print()" style="background-color: #d35400 !important;" class="cursor-pointer hover:opacity-90 text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-md flex items-center gap-2">
    ...
    <div class="invoice-card bg-white rounded-2xl border border-gray-200 shadow-md p-8 max-w-4xl mx-auto relative overflow-hidden font-sans" id="invoice-sheet" style="box-sizing: border-box;">
    ...
    <script>
        async function downloadPDF(e) {
            ...
            const btn = document.getElementById('agent-pdf-btn') || (e ? e.currentTarget : null);
            let origHTML = '';
            if (btn) {
            ...
            try {
                ...
                // Render sheet directly to Canvas
                ...
                // Direct file download to PC Downloads folder
        ...
        window.addEventListener('DOMContentLoaded', () => {
            if (window.location.search.includes('auto_download=1')) {
                setTimeout(() => downloadPDF(), 400);
            } else if (window.location.search.includes('autoprint=1')) {
