...
<body class="bg-[#f0f0f0] min-h-screen flex flex-col">
        ...
        <div class="max-w-5xl mx-auto px-6 pt-6 pb-4">
            <div class="navbar-pill rounded-full px-6 py-3 flex items-center justify-between">
                ...
                <div class="hidden md:flex items-center gap-6">
                    <a href="{{ url('/') }}" class="nav-link flex items-center gap-1.5">
                        <i data-lucide="layout-dashboard" size="14"></i> Dashboard
                    </a>
                    <a href="{{ route('agent.signup') }}" class="nav-link flex items-center gap-1.5" style="color:#fff;">
    ...
    <!-- ─── Floating Card ─── -->
    <div class="flex-1 flex flex-col items-center justify-start px-4 mb-12" style="margin-top: -180px; position: relative; z-index: 10;">
        <div class="card-form bg-white w-full max-w-md p-8 md:p-10" x-data="{ remember: true }">
            ...
            <!-- Flash Messages -->
            @if(session('error'))
                <div class="mb-5 p-4 bg-red-50 border border-red-100 rounded-xl text-red-600 text-xs font-semibold flex items-center gap-2">
            ...
            <form action="{{ route('agent.signup.submit') }}" method="POST" class="space-y-5">
                ...
                <label class="flex items-start gap-3 cursor-pointer group pt-1">
                    <input required type="checkbox" name="terms" class="w-5 h-5 rounded-lg border-gray-300 text-[#e85d26] focus:ring-[#e85d26]/20 transition-all mt-0.5 shrink-0" style="accent-color: #e85d26;" />
                    <span class="text-xs font-semibold text-gray-600 group-hover:text-foreground transition-colors leading-relaxed">
                        I agree to the <a href="{{ url('/terms-and-conditions') }}" class="hover:underline font-bold" style="color: #e85d26;">Terms & Conditions</a> and <a href="{{ url('/privacy-policy') }}" class="hover:underline font-bold" style="color: #e85d26;">Privacy Policy</a>
                    </span>
                </label>
                ...
                <p class="text-xs text-gray-500 text-center pt-2">
                    ...
                    <a href="{{ route('agent.login') }}" class="font-bold hover:underline" style="color:#e85d26;">Sign in</a>
                </p>
            </form>
        </div>
    </div>
