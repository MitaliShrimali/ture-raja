    ...
    <div class="flex min-h-screen" x-data="{ showPassword: false, showForgotPassword: false, forgotEmailSent: false, loading: false }">
        ...
        <div class="w-full lg:w-[45%] flex flex-col justify-center px-8 md:px-16 lg:px-24 py-12 pt-28">
            <div class="max-w-md w-full mx-auto space-y-8">
                ...
                <!-- Main Form -->
                <form x-on:submit="if (showForgotPassword) { $event.preventDefault(); loading = true; forgotEmailSent = false; setTimeout(() => { loading = false; forgotEmailSent = true; }, 1000); }" action="{{ $actionUrl }}" method="POST" class="space-y-6">
                    @csrf
                    
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-gray-700 pl-1">Email<span style="color: #e85d26;">*</span></label>
                        <input required type="email" name="email" value="{{ old('email', $defaultEmail) }}" placeholder="mail@youragency.com" class="w-full bg-white border border-gray-200 rounded-2xl py-4 px-6 outline-none focus:border-[#e85d26]/50 transition-all font-medium text-foreground placeholder:text-gray-400 text-sm shadow-sm" />
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2">
                        <label class="flex items-center gap-3 cursor-pointer group" x-show="!showForgotPassword">
                            <input type="checkbox" name="remember" class="w-5 h-5 rounded-lg border-gray-300 focus:ring-[#e85d26]/20 transition-all" style="accent-color: #e85d26;" checked />
                    ...
                    <button type="submit" class="w-full text-white rounded-2xl py-4 font-bold text-sm shadow-lg transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2" style="background-color: #e85d26; box-shadow: 0 10px 20px rgba(232, 93, 38, 0.2);" onmouseover="this.style.backgroundColor='#d44f1c'" onmouseout="this.style.backgroundColor='#e85d26'">
                        ...
                        <span x-text="showForgotPassword ? (loading ? 'Sending...' : 'Send Reset Link') : '{{ $btnText }}'">{{ $btnText }}</span>
                    </button>

                    <p class="text-xs font-bold text-gray-400 text-center mt-6">
                        Not registered yet? <a href="{{ route('agent.signup') }}" class="hover:underline font-bold" style="color: #e85d26;">Create an Account</a>
                    </p>
                </form>
                <script>
