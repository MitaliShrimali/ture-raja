<?php
$file = 'c:/Users/tusha/Downloads/Tour_raja/resources/views/admin/plans-edit.blade.php';
$content = file_get_contents($file);

$html = <<<'HTML'
<div class="space-y-6">
    <!-- Plan Name (Hidden from standard view to fit in one-liner, but wait, let's just make it all one-liner) -->
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between py-3 border-b border-gray-100">
                    ...
                    <select name="duration" class="w-48 bg-[#F8F9FA] border border-gray-200 rounded-lg py-2 px-3 outline-none text-xs text-right font-semibold text-foreground focus:border-primary">
                        @for ($i = 1; $i <= 365; $i++)
                            <option value="{{ $i }} {{ $i > 1 ? 'Days' : 'Day' }}" {{ ($plan->duration == "$i Days" || $plan->duration == "$i Day" || $plan->duration == $i) ? 'selected' : '' }}>{{ $i }} {{ $i > 1 ? 'Days' : 'Day' }}</option>
                        @endfor
                    </select>
                </div>
                    ...
                    <div class="flex items-center justify-between py-3 border-b border-gray-100">
                        ...
                        @elseif($feat['type'] === 'numeric_dropdown')
                            ...
                            <select name="{{ $inputName }}" {!! $modelName !!} class="w-48 bg-[#F8F9FA] border border-gray-200 rounded-lg py-2 px-3 outline-none text-xs text-right font-semibold text-foreground focus:border-primary cursor-pointer">
                                @for($i = 0; $i <= 50; $i++)
                                    @if($i > 0)
                                    <option value="{{ $i }}" {{ $limitVal == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endif
                                @endfor
